...
export function About() {
  ...
  if (!site) {
    return <section className="bg-white py-20 lg:py-28"><div className="mx-auto max-w-7xl px-4"><div className="h-96 animate-pulse rounded-2xl bg-slate-100" /></div></section>;
  }
        ...
        >
          ...
          >
            {aboutPoints.map((point) => (
              <motion.li key={point.id} variants={listItem} className="flex items-center gap-3 text-sm">
          ...
          >
            <Button asChild className="mt-8 rounded-lg">
              <Link href={site.aboutCtaHref}>
                {site.aboutCtaLabel} <ArrowRight className="size-4" />
              </Link>
            </Button>
          </motion.div>
        </motion.div>
        ...
        >
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-900">{site.aboutCommitmentTitle}</p>
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{site.aboutCommitmentText}</p>
              </div>
            </div>
          </motion.div>
        </motion.div>
