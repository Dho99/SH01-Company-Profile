...
function StatItem({ s, index }: { s: Stat; index: number }) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "flex items-center justify-center gap-4 px-2",
    ...
    >
      <motion.span
        ...
        transition={{ type: "spring", stiffness: 260, damping: 18, delay: index * 0.08 + 0.12 }}
...
export function Stats() {
  ...
  if (stats.length === 0) {
    return <section className="relative z-10 -mt-12 px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl rounded-2xl bg-navy-deep px-6 py-8 shadow-xl ring-1 ring-white/10 animate-pulse h-24" />
    </section>;
  }
